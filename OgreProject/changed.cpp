...
BaseApplication::BaseApplication(void)
	...
	mbLOD(1),
	mbLodThread(1),
	sp(NULL)																				///////////////////Vishesh_change1-> sp initialized to NULL
{
}
...
void BaseApplication::commonCode1()
{
	int j=0;
	Ogre::Entity* arrow;
...
bool BaseApplication::keyPressed( const OIS::KeyEvent &arg )
{
    ...
    else if (arg.key == OIS::KC_R) //reset 
    {
		...
		pathdist = 0;
		disttrav = 0;
		rclick_elemid = -1;		//------------------------------Vishesh_change12---------------------------------------------------
		if_M_Pressed=false;
		overlay->clear();
		import = false;
		totalselectedpoints = 0;
	...
	else if (arg.key == OIS::KC_M)//make interpolated spline and initialise associated variables.
	{
		...
		//Ogre::SceneNode *itemNode = NULL;	//--------------------------------------------------Vishesh_change5----------------------------//
		float temp;
		...
		{			
			...
			//------------------------------------------------------------------------------------------------------------------------
				if(!import)
				{
					...
					// to dynamically modify the path....
					try{
					...
					{
						myManualObject = mSceneMgr->getManualObject("manual1");
						myManualObjectNode = mSceneMgr->getSceneNode("manual1_node");
					}
	...
	else if (arg.key == OIS::KC_L)//palce viewer to ortho view.
	{
		...
		{
			cout<<e.getDescription().c_str();			
		}

		if(sp!=NULL && sp->getNumPoints()!=0)
		{
			if(!clicked_points.empty())
			{
	...
	{
		if(!import && !clicked_points.empty() && sp!=NULL && sp->getNumPoints()>1)	//----------------------------------Vishesh_change8-----------------------------------
			walk = !walk;
		else if(import && !clicked_points.empty())
			walk = !walk;
	...
	else if (arg.key == OIS::KC_A)//aganin start the walk.
	{
		...
		try{
			stopflagtemp.clear();
			stopAtptemp.clear();
			if(stopflag.size()>0&&stopAtp.size()>0){
				for(int i=0;i<stopflag.size();i++)
				{
					//copy
					stopflagtemp.push_back(stopflag[i]);
	...
	else if (arg.key == OIS::KC_E)
    {
		...
		savepath(file);
			...
			//cout<<"Export Failed!!!!"<<e.getDescription().c_str();
		}
    }
	else if (arg.key == OIS::KC_I)
    {
        try
		{
			if(!import && !if_M_Pressed && clicked_points.empty())
			{
				char *file = "C:/Users/root/Desktop/IDH_HAMPI/graphics_project/OgreProject/OgreProject/paths/path_3.txt";
				import = true;
				loadpath(file,false);
			...
			//cout<<"Import Failed!!!!"<<e.getDescription().c_str();
		}
    }
	return true;
}
...
bool BaseApplication::mousePressed( const OIS::MouseEvent &arg, OIS::MouseButtonID id )
{
	...
	case OIS::MB_Left:
		...
		else
		{
			...
			if(!import)
			{
				...
				if(arg.state.X.abs<=500)	//----------------------------------Vishesh_change9-----------------------------------
				{
	...
	case OIS::MB_Right:
		...
		if(rclick_elemid!=-1)
		{
			...
			{
				if(import)
					overlayManager->getOverlayElement(Ogre::String("PanelName_import").append( Ogre::StringConverter::toString(rclick_label)))->setMaterialName("BaseWhite");
				else 
					overlayManager->getOverlayElement(Ogre::String("PanelName").append( Ogre::StringConverter::toString(rclick_elemid+1)))->setMaterialName("BaseWhite");
				rclick_elemid = -1;
				resetBottomPanel();
			}
			catch(Ogre::Exception &e)
...
void BaseApplication::windowResized(Ogre::RenderWindow* rw)
{
	...
	//winsize.x = 0.416*mWindow->getWidth();
	//while(i<clicked_points.size())
	//{
	//	/*arrowNode=mSceneMgr->getSceneNode(Ogre::String("arrownode").append( Ogre::StringConverter::toString(i+1)));
	//	arrowNode->setPosition(convert(arrowNode->getPosition()));*/
